...
class ProjetosAdmModel extends MainModel{
    ...
    public function listar_projetos(){
        ...
        if(is_numeric(chk_array($this->parametros, 0))){
         ...
         //faz consulta
         $query = $this->db->query('SELECT * FROM projeto ' . $where . ' ORDER BY idProjeto DESC' . $query_limit, $id);
         return $query->fetchAll();
    }

    public function obter_projetos() {
        ...
        if ('POST' == $_SERVER['REQUEST_METHOD'] && !empty($_POST['insere_projeto'])) {
            ...
            // Atualiza os dados
            $query = $this->db->update('projeto', 'idProjeto', $projeto_id, $_POST);
	...
	if ($query) {
        ...
        $query = $this->db->query(
                'SELECT * FROM projeto WHERE idProjeto = ? LIMIT 1', array($projeto_id)
        );
        // Obtém os dados
    ...
    public function apaga_projeto(){
        ...
        if(!is_numeric(chk_array($this->parametros, 2)) != 'confirma'){
            ...
            $mensagem .='<a href="'. HOME_URI .'/projetos_adm/">Não</a></p>';
            return $mensagem;
        }
        ...
        //executa a consulta
        $query = $this->db->delete('projeto', 'idProjeto', $projeto_id);
